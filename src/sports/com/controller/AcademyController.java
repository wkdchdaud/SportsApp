...
public class AcademyController {
	...
	public String Accmanag(HttpSession session, HttpServletRequest request, HttpServletResponse response,
		...
		log.info(this.getClass().getName() + "거래처관리 end!");
	...
	public String NoticeInsert(HttpSession session, HttpServletRequest request, HttpServletResponse response, 
		...
		try{
			 ...
			 * */
			String aca_no = CmmUtil.nvl(request.getParameter("aca_no")); //아이디
			String aca_name = CmmUtil.nvl(request.getParameter("aca_name")); //제목
			String aca_area1 = CmmUtil.nvl(request.getParameter("aca_area1")); //공지글 여부
			String aca_area2 = CmmUtil.nvl(request.getParameter("aca_area2")); //공지글 여부
